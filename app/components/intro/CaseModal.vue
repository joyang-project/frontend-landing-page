...
<template>
  <Teleport to="body">
    ...
    >
      <div v-if="selectedCase" class="fixed inset-0 z-[9999] flex items-center justify-center p-4 md:p-8" role="dialog" aria-modal="true">
        ...
        <div class="relative w-full max-w-5xl bg-black rounded-3xl overflow-hidden shadow-2xl ring-1 ring-white/10 flex flex-col md:flex-row max-h-[90vh]">
          <button @click="emit('close')" class="absolute top-4 right-4 z-50 p-2 rounded-full bg-black/50 text-white backdrop-blur-md hover:bg-white hover:text-black transition-all border border-white/10">
            <X class="w-6 h-6" />
          </button>
          ...
          <div class="w-full md:w-[320px] lg:w-[400px] flex-shrink-0 bg-white p-8 flex flex-col justify-center relative">
            <div class="space-y-6">
              <div>
                <span class="inline-block px-3 py-1 rounded-full bg-blue-50 text-blue-700 text-xs font-bold uppercase tracking-widest mb-3">{{ selectedCase.category }}</span>
                <h3 class="text-3xl font-black text-slate-900 leading-tight">{{ selectedCase.title }}</h3>
              </div>
              <div class="w-10 h-1 bg-[#155dfc]"></div>
              <p class="text-slate-700 font-medium leading-relaxed">{{ selectedCase.desc || '고객의 니즈를 정확히 파악하여 최적의 냉난방 시스템을 설계하고 시공했습니다.' }}</p>
